...
namespace Services
{
    public class LinguistSearch : ILinguistSearch
    {
        private readonly IRepository<Employee> empRepository;


        public LinguistSearch(IRepository<Employee> empRepository)
        {
...
order by LinguisticSuppliers.ID");
            ...
            string PreferredObjectValue, string rbJobsObject, string JobsDoneForClient, string ProfessionalBodiesValues, string VendorTypeValues)
        {
            ...
            string JobsDone = "and ((@JobsType is null) or (JobOrders.ContactID = case when @JobsType = 1 then @JobsID end) or (OJ.ID = case when @JobsType = 2 then @JobsID end) or (OGI.ID = case when @JobsType = 3 then @JobsID end))";
            ...
            if (linguistName != null) { linguistName = linguistName.Replace(" ", "%"); };
...
declare @ProfessionalBodiesValues nvarchar(max)
declare @VendorTypeValues nvarchar(max)
...
set @JobsID = '{16}'
set @PostCode = '{17}'
set @ProfessionalBodiesValues = '{18}'
set @VendorTypeValues = '{19}'
...
if @ProfessionalBodiesValues = '' begin set @ProfessionalBodiesValues = null end
if @VendorTypeValues = '' begin set @VendorTypeValues = null end
...
LinguisticSuppliers.EmailAddress, count(distinct jobitems.id) 'Number of items', isnull(LinguisticSupplierStatus.Name,'(none)') 
from LinguisticSuppliers
left outer join LinguisticSupplierTypes on LinguisticSuppliers.SupplierTypeID = LinguisticSupplierTypes.ID
...
left outer join LinguisticSupplierStatus on LinguisticSupplierStatus.ID = Linguisticsuppliers.SupplierStatusID
...
or LinguisticSuppliers.MainContactFirstName +  LinguisticSuppliers.MainContactSurname like '%' + @lingName + '%')
...
and (@ProfessionalBodiesValues is null or dbo.LinguisticSuppliers.ProfessionalBodyID like '%' + @ProfessionalBodiesValues + '%')
and (@VendorTypeValues is null or dbo.LinguisticSuppliers.VendorTypeID like '%' + @VendorTypeValues + '%')
...
LinguisticSupplierTypes.Name 'Type', LinguisticSuppliers.EmailAddress, count(distinct jobitems.id) 'Number of items',
 isnull(LinguisticSupplierStatus.Name,'(none)') 
from LinguisticSuppliers
left outer join LinguisticSupplierTypes on LinguisticSuppliers.SupplierTypeID = LinguisticSupplierTypes.ID
left outer join ExtranetUsers on ExtranetUsers.DataObjectID = LinguisticSuppliers.Id and ExtranetUsers.DataObjectTypeID = 4
left outer join PlanningCalendarAppointments on PlanningCalendarAppointments.ExtranetUserName = ExtranetUsers.UserName
...
rbPreferred, rbJobsObject, JobsDoneForClient, PostcodeValue, ProfessionalBodiesValues,VendorTypeValues, SupplierNDAValue, SupplierEncryptedValue, JobsDone, PrefLinuist);
                    ...
                    while (await result.ReadAsync())
                    {
                        res.Add(new ViewModels.LinguistSearch.LinguistSearchResults()
                        {
                            ...
                            numberOfJobs = await result.GetFieldValueAsync<int>(4),
                            linguistType = await result.GetFieldValueAsync<string>(2),
                            linguistStatus = await result.GetFieldValueAsync<string>(5)
...
set @ProfessionalBodiesValues = '{28}'
set @VendorTypeValues = '{29}'
...
case when LinguisticSupplierRates.AppliesToDataObjectTypeID = 1 then 'only the individual contact' + ' ' + cast(LinguisticSupplierRates.AppliesToDataObjectID as nvarchar) + ' - ' + CA.Name 
 when LinguisticSupplierRates.AppliesToDataObjectTypeID = 2 then 'any contacts at the organisation' + ' ' + cast(LinguisticSupplierRates.AppliesToDataObjectID as nvarchar) + ' - ' + OA.OrgName 
 when LinguisticSupplierRates.AppliesToDataObjectTypeID = 3 then 'any contacts at any organisation within the group' + ' ' + cast(LinguisticSupplierRates.AppliesToDataObjectID as nvarchar) + ' - ' + OGA.Name 
 ...
 LanguageSubjectAreas.Name as 'SA Name', LS.Name as 'LS name', LT.Name as 'LT name',
LanguageServices.Name as 'LS name', LinguisticSupplierStatus.Name as 'Lsup name',  isnull(LinguisticSupplierRates.AppliesToDataObjectTypeID, 0)
from LinguisticSuppliers
left outer join LinguisticSupplierTypes on LinguisticSuppliers.SupplierTypeID = LinguisticSupplierTypes.ID
--left outer join ExtranetUsers on ExtranetUsers.DataObjectID = LinguisticSuppliers.Id and ExtranetUsers.DataObjectTypeID = 4
--left outer join PlanningCalendarAppointments on PlanningCalendarAppointments.ExtranetUserName = ExtranetUsers.UserName
...
left outer join JobOrders on JobOrders.ID = JobItems.Joborderid
left outer join Contacts CJ on CJ.ID = Joborders.ContactID
left outer join Orgs OJ on OJ.ID = CJ.OrgID
left outer join OrgGroups OGI on OGI.ID = OJ.OrgGroupID
...
LinguisticSuppliers.SupplierStatusID, LanguageSubjectAreas.Name, LS.Name , LT.Name, LanguageServices.Name,
LinguisticSupplierStatus.Name, CA.Name, OA.OrgName, OGA.Name, LinguisticSupplierRates.AppliesToDataObjectTypeID
end
else
begin
...
LanguageRateUnits.Name, LinguisticSupplierRates.MinimumCharge, LanguageSubjectAreas.Name, 
LinguisticSupplierRates.AppliesToDataObjectTypeID, LinguisticSupplierRates.AppliesToDataObjectID,Prefix,
LinguisticSuppliers.SupplierStatusID, LanguageSubjectAreas.Name, LS.Name,
LT.Name, LanguageServices.Name, LinguisticSupplierStatus.Name, CA.Name, OA.OrgName, OGA.Name,
LinguisticSupplierRates.AppliesToDataObjectTypeID end", linguistName, EmailValue, KeyWords, MasterFieldsValues, SubFieldsValues, MediaTypesValues,
 supplierStatusValues, CountriesNationalyValues, CountryOfResidenceValues,
SoftwaresValues, gdprStatusListValues, SupplierAvailabilityValues, PreferredObjectValue, rbPreferredObject,
...
SourceLanguagesValues,TargetLanguageValues,SpecialRatesValues,RateUnitsListValues,AppliesToValues,ProfessionalBodiesValues,VendorTypeValues);
                    ...
                    while (await result.ReadAsync())
                    {
                        res.Add(new ViewModels.LinguistSearch.LinguistSearchResults()
                        {
                            ...
                            linguistStatus = await result.GetFieldValueAsync<string>(17),
                            lingService = await result.GetFieldValueAsync<string>(16),
                            lingSourceLang = await result.GetFieldValueAsync<string>(14),
                            lingTargetLang = await result.GetFieldValueAsync<string>(15),
                            lingSubjectArea = await result.GetFieldValueAsync<string>(13),
                            appliesToType = await result.GetFieldValueAsync<byte>(18)
